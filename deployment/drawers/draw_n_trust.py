...
if __name__ == '__main__':
    ...
    n_client = [i*3 for i in range(11)]
    n_client.append(32)
    ...
    plt.plot(n_client[:-1],
            [prot[i][NOT_TRUST_CLIENT_TIME][0]/1000 for i in range(len(n_client))][:-1],
            c='teal',
            label=r'$\mathbb{P}_0$ clients', linewidth=3.0, marker='s', markersize=15)
    plt.plot(n_client[1:],
            [prot[i][PARTIAL_TRUST_CLIENT_TIME][1]/1000 for i in range(len(n_client))][1:],
            c='orange',
    ...
    plt.plot(n_client[1:],
            [(prot[i][TRUST_SGX_CLIENT_TIME][1] + prot[i][TRUST_AMD_CLIENT_TIME][1])/2000 for i in range(len(n_client))][1:],
            c='brown',
